...
MafiaAgentProxy::MafiaAgentProxy(const std::string& name, const std::string& api_url)
    : name_(name), api_url_(api_url), role_(Role::CITIZEN) {
    // Инициализация
}

PlayerAction MafiaAgentProxy::getAction(const std::vector<Player>& players,
...
std::string MafiaAgentProxy::getChatMessage(const std::vector<Player>& players,
                                           const std::vector<ChatMessage>& chat_history,
                                           Phase current_phase) {
    ...
    if (response.empty()) {
        ...
        static const std::vector<std::string> citizen_phrases = {
            ...
            "Я мирный житель, честно!",
            "Ночь была страшной..."
        };
        
        static const std::vector<std::string> mafia_phrases = {
            "Я тоже мирный, как и все.",
            "Кто-то явно врет...",
            "Давайте послушаем аргументы.",
            "Я голосую за него!",
        ...
        if (isMafiaRole(role_)) {
            std::uniform_int_distribution<> dis(0, mafia_phrases.size() - 1);
            return mafia_phrases[dis(gen)];
        } else {
            std::uniform_int_distribution<> dis(0, citizen_phrases.size() - 1);
