...
struct PlayerAction {
    ...
    PlayerAction(Type t = Type::PASS, int target = -1, const std::string& msg = "")
...
struct Player {
    ...
    Player(int id, const std::string& name, Role role) 
        : id(id), name(name), role(role), 
          is_alive(true), is_protected(false), 
          votes_against(0) {}
    
    // Методы для проверки роли
    bool isMafia() const { return role == Role::MAFIA || role == Role::DON; }
    bool isCitizen() const { return role == Role::CITIZEN || role == Role::DOCTOR || role == Role::SHERIFF; }
    bool isSpecial() const { return role == Role::DOCTOR || role == Role::SHERIFF || role == Role::DON; }
    
    std::string getRoleName() const {
        switch(role) {
            case Role::MAFIA: return "Мафия";
            case Role::DON: return "Дон мафии";
            case Role::SHERIFF: return "Шериф";
...
struct ChatMessage {
    ...
    ChatMessage(int pid, const std::string& name, const std::string& role, 
                const std::string& txt, const std::string& time, 
                bool night, bool pub = true)
        : player_id(pid), player_name(name), player_role(role), 
          text(txt), timestamp(time), is_night(night), is_public(pub) {}
    
    std::string toString() const {
        std::string time_of_day = is_night ? "🌙" : "☀️";
        return "[" + timestamp + "] " + time_of_day + " " + 
               player_name + " (" + player_role + "): " + text;
    }
};
...
public:
    ...
    virtual PlayerAction getAction(const std::vector<Player>& players,
    ...
    virtual std::string getChatMessage(const std::vector<Player>& players,
                                      const std::vector<ChatMessage>& chat_history,
...
public:
    ...
    void addChatMessage(int player_id, const std::string& message, bool is_night, bool is_public = true);
    
    // Получить историю чата
    const std::vector<ChatMessage>& getChatHistory() const { return chat_history_; }
...
private:
    ...
    void sendPrivateMessage(int player_id, const std::string& message, bool is_night = false);
