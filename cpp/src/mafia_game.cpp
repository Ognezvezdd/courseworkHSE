...
bool MafiaGame::initialize(const std::vector<std::shared_ptr<IMafiaAgent>>& agents) {
    if (agents.size() < 6) {
    ...
    for (size_t i = 0; i < agents_.size(); ++i) {
        // Сначала создаем с временной ролью CITIZEN
...
void MafiaGame::assignRoles() {
    ...
    for (size_t i = 0; i < players_.size(); ++i) {
        ...
        if (roles[i] == Role::SHERIFF) sheriff_id_ = i;
        if (roles[i] == Role::DOCTOR) doctor_id_ = i;
        if (roles[i] == Role::DON) don_id_ = i;
...
void MafiaGame::executeNightPhase() {
    ...
    for (int mafia_id : getAliveMafiaIds()) {
        ...
        PlayerAction action = agents_[mafia_id]->getAction(
            players_, chat_history_, current_phase_, players_[mafia_id].known_info);
        
        if (action.type == PlayerAction::Type::MAFIA_KILL && action.target_id != -1) {
            night_actions_[mafia_id] = action;
        }
    }
    
    // 2. Дон проверяет (если жив)
    if (don_id_ != -1 && players_[don_id_].is_alive) {
        current_phase_ = Phase::NIGHT_DON;
        sendPrivateMessage(don_id_, "Дон, кого вы хотите проверить?", true);
        
        PlayerAction action = agents_[don_id_]->getAction(
            players_, chat_history_, current_phase_, players_[don_id_].known_info);
        
        if (action.type == PlayerAction::Type::DON_CHECK && action.target_id != -1) {
            night_actions_[don_id_] = action;
        }
    }
    
    // 3. Шериф проверяет (если жив)
    if (sheriff_id_ != -1 && players_[sheriff_id_].is_alive) {
        current_phase_ = Phase::NIGHT_SHERIFF;
        sendPrivateMessage(sheriff_id_, "Шериф, кого вы хотите проверить?", true);
        
        PlayerAction action = agents_[sheriff_id_]->getAction(
            players_, chat_history_, current_phase_, players_[sheriff_id_].known_info);
        
        if (action.type == PlayerAction::Type::SHERIFF_CHECK && action.target_id != -1) {
            night_actions_[sheriff_id_] = action;
        }
    }
    
    // 4. Доктор лечит (если жив)
    if (doctor_id_ != -1 && players_[doctor_id_].is_alive) {
        current_phase_ = Phase::NIGHT_DOCTOR;
        sendPrivateMessage(doctor_id_, "Доктор, кого вы хотите вылечить?", true);
        
        PlayerAction action = agents_[doctor_id_]->getAction(
            players_, chat_history_, current_phase_, players_[doctor_id_].known_info);
        
        if (action.type == PlayerAction::Type::DOCTOR_HEAL && action.target_id != -1) {
...
void MafiaGame::resolveNightActions() {
    ...
    if (doctor_id_ != -1 && players_[doctor_id_].is_alive && 
        ...
        if (heal_target == kill_target) {
            is_protected = true;
            players_[kill_target].is_protected = true;
            sendPrivateMessage(doctor_id_, 
                "Вы успешно защитили " + players_[kill_target].name + "!", true);
        }
    }
    ...
    if (kill_target != -1 && !is_protected) {
        ...
        broadcastMessage("Ночью мафия убила " + players_[kill_target].name + "!", false);
    } else if (kill_target != -1 && is_protected) {
        broadcastMessage("Мафия пыталась убить кого-то, но доктор спас жертву!", false);
    } else {
    ...
    if (sheriff_id_ != -1 && players_[sheriff_id_].is_alive && 
        ...
        if (check_target != -1) {
            bool is_mafia = players_[check_target].isMafia();
            std::string result = is_mafia ? "является мафией" : "не является мафией";
            sendPrivateMessage(sheriff_id_, 
                players_[check_target].name + " " + result, true);
            players_[sheriff_id_].known_info.push_back(
                "Проверка: " + players_[check_target].name + " " + result);
        }
    }
    ...
    if (don_id_ != -1 && players_[don_id_].is_alive && 
        ...
        if (check_target != -1) {
            bool is_sheriff = players_[check_target].role == Role::SHERIFF;
            std::string result = is_sheriff ? "является шерифом" : "не является шерифом";
            sendPrivateMessage(don_id_, 
                players_[check_target].name + " " + result, true);
            players_[don_id_].known_info.push_back(
                "Проверка: " + players_[check_target].name + " " + result);
        }
    }
...
void MafiaGame::executeDayPhase() {
    // Фаза обсуждения
    current_phase_ = Phase::DAY_DISCUSSION;
    broadcastMessage("Начинается дневное обсуждение. У вас есть время на дискуссию.", false);
    ...
    broadcastMessage("Обсуждение завершено. Начинается голосование за изгнание.", false);
    ...
    for (const auto& player : players_) {
        if (player.is_alive) {
            ...
            if (action.type == PlayerAction::Type::VOTE_KILL && action.target_id != -1) {
                ...
                if (target_idx != -1 && players_[target_idx].is_alive) {
                    ...
                    addChatMessage(player.id, 
                        "голосует против " + players_[target_idx].name, false, true);
                }
            }
        }
    }
...
void MafiaGame::resolveVoting() {
    ...
    if (tie || max_votes == 0) {
        ...
        if (target_idx != -1) {
            std::string role_reveal = " (роль: " + players_[target_idx].getRoleName() + ")";
            killPlayer(exile_target, "изгнан по результатам голосования");
            broadcastMessage(players_[target_idx].name + role_reveal + " изгнан по результатам голосования!", false);
        }
    }
...
bool MafiaGame::checkWinCondition() {
    ...
    if (alive_mafia == 0) {
        ...
        broadcastMessage("🎉 Мирные жители победили! Все мафиози найдены и наказаны.", false);
        return true;
    }
    ...
    if (alive_mafia >= alive_citizens) {
        ...
        broadcastMessage("💀 Мафия победила! Мирные жители не смогли найти всех преступников.", false);
        return true;
    }
...
void MafiaGame::killPlayer(int player_id, const std::string& reason) {
    ...
    if (player_id == sheriff_id_) sheriff_id_ = -1;
    if (player_id == doctor_id_) doctor_id_ = -1;
    if (player_id == don_id_) don_id_ = -1;
}

void MafiaGame::addChatMessage(int player_id, const std::string& message, 
                              bool is_night, bool is_public) {
    int idx = findPlayerById(player_id);
    if (idx == -1 || !players_[idx].is_alive) return;
    
    std::string timestamp = getCurrentTime();
    std::string role_name = players_[idx].getRoleName();
    
    ChatMessage msg(player_id, players_[idx].name, role_name, 
                   message, timestamp, is_night, is_public);
...
void MafiaGame::broadcastMessage(const std::string& message, bool is_night) {
    std::string timestamp = getCurrentTime();
    ChatMessage msg(-1, "Система", "Системное", 
                   message, timestamp, is_night, true);
    chat_history_.push_back(msg);
    game_log_.push_back(msg.toString());
}

void MafiaGame::sendPrivateMessage(int player_id, const std::string& message, bool is_night) {
    int idx = findPlayerById(player_id);
    if (idx == -1) return;
    
    std::string timestamp = getCurrentTime();
    ChatMessage msg(-1, "Система", "Системное", 
                   message, timestamp, is_night, false);
...
MafiaResult MafiaGame::getResult() const {
    ...
    for (const auto& player : players_) {
        if (player.is_alive) result.surviving_players++;
    }
...
namespace RoleUtils {
    ...
    Role stringToRole(const std::string& role_str) {
        if (role_str == "mafia") return Role::MAFIA;
        if (role_str == "don") return Role::DON;
        if (role_str == "sheriff") return Role::SHERIFF;
        if (role_str == "doctor") return Role::DOCTOR;
        if (role_str == "citizen") return Role::CITIZEN;
    ...
    std::string getRoleDescription(Role role) {
        switch(role) {
            case Role::MAFIA: return "Мафия: знает других мафиози, ночью голосует за убийство";
            case Role::DON: return "Дон мафии: знает мафию, может проверять игроков ночью";
            case Role::SHERIFF: return "Шериф: ночью может проверить игрока на причастность к мафии";
            case Role::DOCTOR: return "Доктор: ночью может защитить одного игрока от убийства";
            case Role::CITIZEN: return "Мирный житель: не имеет специальных способностей, голосует днем";
            default: return "Неизвестная роль";
        }
    }
    
    int getRecommendedMafiaCount(int total_players) {
        if (total_players <= 6) return 1;
        if (total_players <= 8) return 2;
        if (total_players <= 10) return 3;
