...
bool MafiaGame::initialize(
  ...
  for (size_t i = 0; i < agents_.size(); ++i) {
    if (!agents_[i]) {
      ...
      game_log_.push_back("Количество игроков: " +
    ...
    void MafiaGame::assignRoles() {
      ...
      for (size_t i = 0; i < players_.size(); ++i) {
        ...
        game_log_.push_back(players_[i].name + " - " +
                            players_[i].getRoleName());
      }
    }
    ...
    void MafiaGame::executeNightPhase() {
      ...
      if (don_id_ != -1) {
        ...
        if (don_idx != -1 && players_[don_idx].is_alive) {
          ...
          PlayerAction action = agents_[don_idx]->getAction(
      ...
      if (sheriff_id_ != -1) {
        ...
        if (sheriff_idx != -1 && players_[sheriff_idx].is_alive) {
          ...
          sendPrivateMessage(sheriff_id_, "Шериф, кого вы хотите проверить?",
                             true);
      ...
      if (doctor_id_ != -1) {
        ...
        if (doctor_idx != -1 && players_[doctor_idx].is_alive) {
          ...
          sendPrivateMessage(doctor_id_, "Доктор, кого вы хотите вылечить?",
                             true);
    ...
    void MafiaGame::resolveNightActions() {
      ...
      if (kill_target != -1 && !is_protected) {
        ...
        if (target_idx != -1) {
          ...
          broadcastMessage(
              "Ночью мафия убила " + players_[target_idx].name + "!", false);
        }
    ...
    void MafiaGame::executeDayPhase() {
      ...
      current_phase_ = Phase::DAY_DISCUSSION;
      broadcastMessage(
          "Начинается дневное обсуждение. У вас есть время на дискуссию.",
          false);
      ...
      broadcastMessage(
          "Обсуждение завершено. Начинается голосование за изгнание.", false);
          ...
          if (action.type == PlayerAction::Type::VOTE_KILL &&
            ...
            if (target_idx != -1 && players_[target_idx].is_alive) {
              ...
              addChatMessage(player.id,
                             "голосует против " + players_[target_idx].name,
                             false, true);
            }
          }
        }
    ...
    void MafiaGame::resolveVoting() {
      ...
      if (tie || max_votes == 0) {
        broadcastMessage("Голосование закончилось вничью. Никто не изгнан.",
                         false);
      } else {
    ...
    bool MafiaGame::checkWinCondition() {
      ...
      if (alive_mafia == 0) {
        ...
        broadcastMessage(
            "🎉 Мирные жители победили! Все мафиози найдены и наказаны.",
            false);
        return true;
      }
    ...
    void MafiaGame::addChatMessage(int player_id, const std::string &message,
      ...
      ChatMessage msg(player_id, players_[idx].name, role_name, message,
                      timestamp, is_night, is_public);
    ...
    void MafiaGame::broadcastMessage(const std::string &message,
                                     bool is_night) {
      std::string timestamp = getCurrentTime();
    ...
    void MafiaGame::sendPrivateMessage(
        int player_id, const std::string &message, bool is_night) {
      int idx = findPlayerById(player_id);
      if (idx == -1)
        return;
    ...
    MafiaResult MafiaGame::getResult() const {
      ...
      for (const auto &player : players_) {
        if (player.isMafia()) {
          result.mafia_team.push_back(player.name + " (" +
                                      player.getRoleName() + ")");
        } else {
          result.citizen_team.push_back(player.name + " (" +
                                        player.getRoleName() + ")");
        }
      }
    ...
    bool isCitizenRole(Role role) {
      return role == Role::CITIZEN || role == Role::SHERIFF ||
             role == Role::DOCTOR;
    }
